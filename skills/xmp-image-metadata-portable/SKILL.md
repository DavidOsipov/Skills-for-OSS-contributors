...
All scripts share `scripts/_common.py` (ExifTool discovery, version parsing,
argument-injection-safe paths, XML-safe escaping). They are fully type-annotated and
pass `mypy --disallow-untyped-defs` and `ruff` (config in `pyproject.toml`), with a
...
Hardening applied: file paths are absolutised before reaching ExifTool (a dash-leading
filename can no longer be read as an option); all spec/profile text is stripped of
XML-illegal control characters and escaped for element text and attributes; every
subprocess call uses list arguments only (never a shell); timestamps are timezone-aware;
the profile is validated (unfilled template placeholders and unknown licenses are
rejected); geocoding requires HTTPS and explicit opt-in.
